...
"""
Base Genetic Algorithm with Integer Encoding
...
class IntGA:
    ...
    def __init__(self, init_pop = 2, dim = 1, max_cycles = 1, opc = 1,):
        ...
        if(self.opc < 4):
            ...
            self.population = [
                {
                    'genes':
                    [rand.randrange(self.min, self.max) for i in range(self.dim)],
                    'eval': 0.,
                    'x': [rand.randrange(self.min, self.max) for i in range(self.dim)],
                    'prob': 0.,
                    'fitness': 0.
    ...
    def crossover(self):
        for (i, ind) in enumerate(self.selected):
            ...
            if self.opc < 4:
                ...
                cross_point = rand.randrange(len(ind['x']))
    ...
    def mutation(self):
        ...
        for child in self.children:
            rand_gene = rand.randrange(self.dim)

            r = rand.random()

            if self.opc < 4:
                if r <= rate:
                    child['x'][rand_gene] = rand.randrange(self.min, self.max)
            else:
                if r <= rate:
...
def main():
    ...
    with open('res_int_'+str(d.opc)+'_10.csv', mode='w') as res_file:
